...
RUN sed -i "/ors.engine.source_file=.*/s/.*/ors.engine.source_file=\/home\/ors\/files\/sumatra-latest.osm.pbf/" "/tmp/ors/example-ors-config.env" && \
        sed -i "/    source_file:.*/s/.*/    source_file: \/home\/ors\/files\/sumatra-latest.osm.pbf/" "/tmp/ors/example-ors-config.yml"
...
ARG UID=1000
ARG GID=1000
ARG OSM_FILE=./ors-api/src/test/files/sumatra-latest.osm.pbf
...
COPY --chown=ors:ors ./$OSM_FILE /sumatra-latest.osm.pbf
